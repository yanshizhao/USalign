...
// [Coords& overload] xcentroids non-const (do_rotation), ycentroids const (read-only)
double homo_refined_greedy_search(double **TMave_mat,int *assign1_list,
    int *assign2_list, const int chain1_num, const int chain2_num,
...
// [Coords& + Rotation overload] ut_mat containerized
double homo_refined_greedy_search(double **TMave_mat,int *assign1_list,
    int *assign2_list, const int chain1_num, const int chain2_num,
    Coords& xcentroids, const Coords& ycentroids, const double d0MM,
    ...
    const vector<string>&resi_vec1, const vector<string>&resi_vec2)
{
    ...
    MMalign_final(xname, yname, chainID_list1, chainID_list2,
...
// [Coords& true implementation]
    ...
    if (!bAlignStick)
    {
        ...
        TM = DP_iter_dimer(r1, r2, xtm, ytm, xt, path, vv.data(), xa, ya, xlen, ylen,
        ...
        /************************************************************/
        get_initial_ss_dimer(path, vv.data(), secx, secy, xlen, ylen, mask, invmap);
        ...
        if (TM > TMmax*0.2)
        {
            TM = DP_iter_dimer(r1, r2, xtm, ytm, xt, path, vv.data(), xa, ya,
        ...
        if (get_initial5_dimer( r1, r2, xtm, ytm, path, vv.data(), xa, ya,
        ...
        get_initial_ssplus_dimer(r1, r2, sv.data(), path, vv.data(), secx, secy, xa_c, ya_c,
